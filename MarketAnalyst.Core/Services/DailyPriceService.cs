...
namespace MarketAnalyst.Core.Services
{
    ...
    {
        Task<bool> CalculateDailyPrice();
    }
    public class DailyPriceService : IDailyPriceService
    {
        private readonly IServiceScopeFactory _scopeFactory; 
        public DailyPriceService(IServiceScopeFactory serviceFactory)
        {
            _scopeFactory = serviceFactory; 
        }
        public async Task<bool> CalculateDailyPrice()
        {
                    ...
                    if (!string.IsNullOrEmpty(stock.UniqueCode))
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(history))
                            {
                                ...
                                var previouslySavedDates = await unitOfWork.StocksDailyPriceService.GetDates(stock.Id);
                                ...
                                {
                                    await unitOfWork.StocksDailyPriceService.AddRange(priceListToAdd);
                                    await unitOfWork.SaveAsync();
                                }
                                Data.General.Setting.LastDailyPriceCalculation = DateTime.Now;
                            }
                        }
                        catch (Exception exc)
                        {
